...
class ChromeEndpointTests(EndpointDataTestCase):
    ...
    def test_alert_window_boundary(self):
        ...
        settings_obj.save()
        self.assertEqual(
            self.client.get("/api/v1/chrome").json()["alert"], {"text": "窗口提示"}
        )
...
class ArchiveEndpointTests(EndpointDataTestCase):
    ...
    def test_unknown_section_404(self):
        self.assertEqual(
            self.client.get("/api/v1/sections/bogus/archive").status_code, 404
        )
...
class SearchEndpointTests(EndpointDataTestCase):
    ...
    def test_historical_visibility(self):
        ...
        expired = make_notice(
            self.jwc_chronicle, slug="s-expired", title="搜索到期", publish=True
        )
    ...
    def test_merged_order_by_first_published_desc(self):
        ...
        titles = [
            e["title"]
            for e in self.client.get("/api/v1/search?dept=jwc").json()["entries"]
        ]
...
class DetailEndpointTests(EndpointDataTestCase):
    ...
    def test_notice_detail_all_six_blocks(self):
        ...
        document = Document.objects.create(
            ...
            file=SimpleUploadedFile(
                "manual.pdf", b"%PDF-1.4 fake", content_type="application/pdf"
            ),
        )
    ...
    def test_material_software_guide_details(self):
        ...
        material = make_material(
            materials, slug="d-material", title="详情资料", publish=True
        )
    ...
    def test_expired_detail_allowed_with_flag(self):
        expired = make_notice(
            self.jwc_chronicle, slug="d-expired", title="到期详情", publish=True
        )
        expire(expired)
    ...
    def test_scheduled_not_yet_live_404(self):
        ...
        self.assertEqual(
            self.client.get(f"/api/v1/pages/{_path(scheduled)}").status_code, 404
        )

    def test_unpublished_404(self):
        """S4 下线 404。"""
        notice = make_notice(
            self.jwc_chronicle, slug="d-unpub", title="下线详情", publish=True
        )
        notice.unpublish()
        self.assertEqual(
            self.client.get(f"/api/v1/pages/{_path(notice)}").status_code, 404
        )

    def test_missing_ancestors_404(self):
        base = "/api/v1/pages"
        self.assertEqual(
            self.client.get(f"{base}/bogus/jwc/ep-notice").status_code, 404
        )
        self.assertEqual(
            self.client.get(f"{base}/chronicle/nobody/ep-notice").status_code, 404
        )
        self.assertEqual(
            self.client.get(f"{base}/chronicle/jwc/missing").status_code, 404
        )
    ...
    def test_draft_container_unreachable(self):
        ...
        self.assertEqual(
            self.client.get(
                "/api/v1/pages/chronicle/draft-dept/hidden-in-draft"
            ).status_code,
            404,
        )
...
class SitemapEndpointTests(EndpointDataTestCase):
    ...
    def test_live_public_semantics_and_lastmod(self):
        ...
        expired = make_notice(
            self.jwc_chronicle, slug="sm-expired", title="站点到期", publish=True
        )
