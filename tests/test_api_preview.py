...
from .helpers import build_sections, make_container, make_department, make_notice, notice_data
...
class PreviewFlowTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.notice = make_notice(
            cls.container, slug="prev-notice", title="预览通知", publish=True
        )
...
class MintEndpointTests(PreviewFlowTestCase):
    def test_mint_binds_session_user_page(self):
        ...
        self.assertEqual(
            payload["s"], self.admin_client.session.session_key
        )
...
class RedemptionTests(PreviewFlowTestCase):
    def test_redemption_happy_path(self):
        ...
        published = self.client.get(
            f"/api/v1/pages/{self.notice.url.strip('/')}"
        ).json()
    ...
    def test_redemption_data_is_preview_form_path_rebuild(self):
        ...
        obj = self.notice.get_latest_revision_as_object()
        data = urlencode(
            notice_data(self.container, title="表单路径草稿", summary="表单路径摘要")
        )
