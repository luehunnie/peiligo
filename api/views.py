...
def mint_token(session_key, user_pk, page_pk):
    ...
    return signing.dumps(
        {"s": session_key, "u": user_pk, "p": page_pk}, salt=PREVIEW_TOKEN_SALT
    )
    ...
    return JsonResponse(
        {
            ...
            "nav_sections": [
                {"slug": section["slug"], "title": section["title"], "url": f"/{section['slug']}/"}
    ...
    return JsonResponse(
        {
            ...
            "alert": _active_alert(SiteSettings.for_request(request), now),
            "carousel": [
                serializers.carousel_entry(entry) for entry in _carousel_entries(root.pk)
            ],
            "sections": [
...
def section_archive(request, slug):
    ...
    return JsonResponse(
        {
            "section": {"slug": section.slug, "title": section.title, "url": section.url},
...
def _resolve_content_page(section, dept, slug):
    ...
    if section_page is None:
        raise Http404
    container = (
        section_page.get_children()
        .type(DepartmentContainerPage)
        .live()
        .filter(slug=dept)
        .first()
    )
...
def preview(request):
    ...
    try:
        payload = signing.loads(
            token, salt=PREVIEW_TOKEN_SALT, max_age=PREVIEW_TOKEN_MAX_AGE
        )
    ...
    if not isinstance(specific, SEARCHABLE_PAGE_MODELS):
        raise Http404
    return JsonResponse(
        serializers.page_detail_data(_rebuild_draft(specific, user), preview=True)
    )
