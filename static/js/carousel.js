/**
 * 首页轮播渐进增强（首页升级 Phase 7；ADR-0007：SSR-first ＋ 最小原生 JS）。
 *
 * 契约（不可破坏）：
 * - 内容事实源始终是 SSR DOM（[data-carousel] / [data-carousel-item]）；
 *   本脚本只控制 visibility、controls、timing，不改写任何 href/内容。
 * - 无 JS：SSR 条目全部按文档流自然可达（零改动）；初始化若在改写 SSR
 *   可见性之后抛错，立即 rollback 为等价于「未增强」的全可见状态。
 * - 0 项：SSR 本不应输出区块，异常空容器安全 no-op；
 *   1 项：保持静态（无 autoplay / 箭头 / 圆点 / hidden / 增强标记）；
 *   2–5 项：全部准备（对象/控件/事件）完成后才事务式提交（隐藏非 active
 *   项→控件→标记→autoplay），提交阶段任一步骤失败即 rollback＋安全 return。
 * - 安全边界（TASK O）：无 eval / new Function / document.write / innerHTML、
 *   不读取 URL 参数、不 fetch、不 storage、无第三方脚本、无 inline handler。
 */
(function () {
    "use strict";

    // PRD §10：自动切换间隔约 5–6 秒，冻结 5500ms（仅 ≥2 项允许 autoplay）。
    var AUTOPLAY_INTERVAL_MS = 5500;
    ...
    function initCarousel(carousel) {
        ...
        var index = 0;
        var dots = [];
        var enhanced = false; // 增强态闸门：回滚后所有 handler/autoplay 安全 no-op
        ...
        var pageHidden = document.hidden; // 页面在隐藏态加载时同样不 autoplay
