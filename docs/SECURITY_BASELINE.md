...
> **历史状态注记（2026-09-12，Phase 9.5 复核）**：本文是 M7.1 时点的**设计基线 + 现状审计**，全文以 main=cd6eca0 代码为「现状」基准；其 `NOT_IMPLEMENTED` / `NEEDS_VERIFICATION` 标签均为**当时结论**，作为历史记录保留、不再逐条改写。其中多项控制在后续阶段已落地——登录防爆破（django-axes）、附件三级校验、外链确认跳转页、生产 HSTS / CSP / Secure Cookie、DEBUG=False 等均已实现（现状与证据见 [guides/TECHNICAL_ARCHITECTURE_GUIDE.md](guides/TECHNICAL_ARCHITECTURE_GUIDE.md) 安全章节与 Phase 9 发布安全批次）。需要核对**当前**安全状态时，请以该指南为准，不要把本文的状态标签当作现状。
