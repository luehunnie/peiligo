<!-- adr-provenance
恢复来源：luehunnie/peiligo PR #19（closed，未合并）head 7df6dc05d7de81fedf52456a796b833250433d8d
          的 GitHub 存档 diff（原分支删除后唯一证据源）；基线＝48fd5d6。
恢复日期：2026-09-16。
逐字节 SHA-256（恢复源、即本文件正文自本注释块之后的部分）：
          6e9ad3b9e8871e219686732742f426446581eb08fff4b2103850333acd02fe11
状态说明：正文为上游 verbatim 恢复（含 Accepted 状态字段头，Human 2026-09-15 G2 批准）。
          文中提及的 docs/adr/README.md（ADR 机制/索引/看板）属 peiligo 仓库治理
          载体，不在本仓库恢复范围；本仓库内本 ADR 作为 B02 overlay 的契约决策
          证据持久化（相关联的机器契约快照＝docs/api/openapi.json，溯源见其 README）。
维护规则：本文件为冻结证据，不做就地编辑；契约演进走上游 peiligo 后按其新版本
          重新恢复（刷新本块 SHA-256 与来源）。
-->
