...
| `SCHED_INTERVAL_SECONDS` | ◻️ | 定时发布调度间隔,缺省 `60` 秒;**生产切换 Gate 5 验收条件:须 ≤`30` 且 scheduler 容器运行中**(缺省 60 仅满足 ≤5min 允许窗口,不满足 ≤30s 目标——Headless API 契约 F3,不达标即生产切换 blocker) | `60` | 否 |
